...
def get_all_melds(hand: List[Card]) -> List[List[Card]]:
    ...
    all_melds = get_all_run_melds(hand) + get_all_set_melds(hand)
    return all_melds
...
#     if len(hand) != 10:
#         raise DummyProgramError("Hand contain {} cards: should be 10 cards.".format(len(hand)))
#     result = []  # type: List[List[List[Card]]]
...
def check_can_deposit(card : Card, meld: List[Card]):
    if len(meld) < 3:
        return False
    meld = sorted(meld, key=utils.get_card_id)
    if meld[0].rank == meld[1].rank: # set
        return meld[0].rank == card.rank
         
    else:
        return  (utils.get_card_id(meld[0]) == utils.get_card_id(card) + 1  or utils.get_card_id(meld[len(meld) - 1]) == utils.get_card_id(card) - 1) and meld[0].suit == card.suit
